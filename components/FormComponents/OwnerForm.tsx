...
import { validateName , validatePhoneNo , validateNIC , validateAccountNo , validateBankName , validateBranchName , validateOTP} from "./FormFunctions";
...
export const OwnerFormPage1 = ({fname, setFName, lname, setLName, phoneNo, setPhoneNo, nic, setNIC, setNextVisible, setBackVisible, currentPos}: any) => {
  const [fnameError, setFnameError] = useState(false);
  const [lnameError, setLnameError] = useState(false);
  const [phoneNoError, setPhoneNoError] = useState(false);
  const [nicError, setNICError] = useState(false);
  const inputRefs = useRef(Array.from({ length: 4 }, () => React.createRef<TextInput>()));
...
export const OwnerFormPage2 = ({accountNo, setAccountNo, accHolderName, setAccHolderName, bankName, setBankName, branchName, setBranchName, setNextVisible, setBackVisible, currentPos}: any) => {
  ...
  // Make "Next" button visible
  useEffect (() => {
    if (!accNoError && !accHolderNameError && !bankNameError && !branchNameError && accountNo != '' && accHolderName != '' && bankName != '' && branchName != '') {
      setNextVisible(true);
    } else {
      setNextVisible(false);
    }
  ...
  return (
    <GestureHandlerRootView>
      <FormInput ref={inputRefs.current[0]} nextFocus={inputRefs.current[1]} title="Account No" input={accountNo} setInput={setAccountNo} error={accNoError} setError={setAccNoError} errorMessage={'Invalid account number'} validation={validateAccountNo} maxLength={12} placeholder="0000 0000 0000" />
      <FormInput ref={inputRefs.current[1]} nextFocus={inputRefs.current[2]} title="Name of Account Holder" input={accHolderName} setInput={setAccHolderName} error={accHolderNameError} setError={setAccHolderNameError} errorMessage={'Invalid name'} validation={validateName} maxLength={256} placeholder="Mr. John Doe" />
      <FormInput ref={inputRefs.current[2]} nextFocus={inputRefs.current[3]} title="Bank name" input={bankName} setInput={setBankName} error={bankNameError} setError={setBankNameError} errorMessage={'Invalid bank name'} validation={validateName} maxLength={256} placeholder="Bank name" />
      <FormInput ref={inputRefs.current[3]} title="Branch name" input={branchName} setInput={setBranchName} error={branchNameError} setError={setBranchNameError} errorMessage={'Invalid branch name'} validation={validateName} maxLength={256} placeholder="Branch name" />
    </GestureHandlerRootView>
  );
}
...
export const OwnerFormPage3 = ({otp, setOTP, nextAction, setNextVisible, setBackVisible, currentPos}: any) => {
