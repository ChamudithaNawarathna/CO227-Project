...
export const PassengerFormPage1 = ({fname, setFName, lname, setLName, phoneNo, setPhoneNo, setNextVisible, setBackVisible, currentPos}: any) => {
  ...
  useEffect (() => {
    if (!fnameError && !lnameError && !phoneNoError && fname != '' && lname != '' && phoneNo != '') {
      setNextVisible(true);
    } else {
      setNextVisible(false);
    }
  }, [fname, lname, phoneNo, currentPos]);
  ...
  return (
    <GestureHandlerRootView>
      ...
      <FormInput ref={inputRefs.current[2]} title="Phone Number" input={phoneNo} setInput={setPhoneNo} error={phoneNoError} setError={setPhoneNoError} errorMessage={'Invalid phone number'} keyboardType='number-pad' validation={validatePhoneNo} maxLength={10} placeholder="Phone number" />
    </GestureHandlerRootView>
  );
}
...
export const PassengerFormPage2 = ({otp, setOTP, nextAction, setNextVisible, setBackVisible, currentPos}: any) => {
